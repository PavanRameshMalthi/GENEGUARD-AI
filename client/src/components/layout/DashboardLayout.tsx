...
const DashboardLayout: React.FC<DashboardLayoutProps> = ({ 
  ...
  return (
    <div className="min-h-screen bg-[#0e1428] dark:bg-[#060913] bg-[radial-gradient(ellipse_100%_80%_at_50%_-20%,rgba(99,102,241,0.22),rgba(14,20,40,0))] dark:bg-[radial-gradient(ellipse_100%_80%_at_50%_-20%,rgba(79,70,229,0.18),rgba(6,9,19,0))] flex flex-col items-center justify-center p-0 lg:p-6 transition-colors duration-300 relative overflow-x-hidden antialiased selection:bg-indigo-500 selection:text-white">
      {/* Ambient background glow effects */}
      <div className="hidden lg:block absolute top-12 left-12 w-96 h-96 bg-indigo-500/10 rounded-full blur-3xl pointer-events-none" />
      <div className="hidden lg:block absolute bottom-12 right-12 w-96 h-96 bg-purple-500/10 rounded-full blur-3xl pointer-events-none" />

      {/* Floating Elevated Dashboard Container */}
      <div className="w-full lg:max-w-[1540px] min-h-screen lg:min-h-[calc(100vh-48px)] my-0 lg:my-auto bg-slate-50/98 dark:bg-[#0c1022]/98 lg:rounded-[32px] border-0 lg:border border-white/20 dark:border-indigo-950/50 shadow-none lg:shadow-[0_30px_70px_-15px_rgba(0,0,0,0.5)] flex flex-row overflow-hidden backdrop-blur-xl relative transition-all duration-300">
        
        {/* Left Collapsible Sidebar */}
        <Sidebar />

        {/* Main Content & Header Area */}
        <div className="flex-1 flex flex-col min-w-0 min-h-full overflow-y-auto custom-scrollbar transition-all duration-300">
          {/* Top Dashboard Header */}
          <header className="bg-white/90 dark:bg-[#0d1226]/90 backdrop-blur-xl border-b border-slate-200/80 dark:border-slate-800/70 min-h-[72px] flex items-center justify-between px-4 sm:px-6 lg:px-8 shrink-0 sticky top-0 z-20 transition-all duration-300">
