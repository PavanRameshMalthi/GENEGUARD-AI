...
export const analyzeHealthData = async (req: any, res: Response) => {
  try {
    ...
    if (assessmentId) {
      ...
      if (!assessment.calculations || !assessment.calculations.healthScore) {
        const input: CalculationInput = {
          ...
          medicalConditions: Object.keys(assessment.medicalHistory || {}).filter(k => (assessment.medicalHistory as any)[k] === true),
          familyConditions: Object.keys(assessment.familyHistory || {}).filter(k => (assessment.familyHistory as any)[k] === true),
