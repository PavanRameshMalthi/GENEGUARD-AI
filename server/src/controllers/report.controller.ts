...
export const getComprehensiveHealthReport = async (req: any, res: Response) => {
  try {
...
export const compareReports = async (req: any, res: Response) => {
  try {
    const userId = req.user._id;
...
export const uploadReport = async (req: any, res: Response) => {
  try {
    ...
    if (lowerName.includes('blood') || lowerName.includes('cbc')) reportType = 'Blood Test';
    else if (lowerName.includes('lipid') || lowerName.includes('cholesterol')) reportType = 'Lipid Profile';
    else if (lowerName.includes('sugar') || lowerName.includes('glucose') || lowerName.includes('hba1c')) reportType = 'Diabetes Panel';
    else if (lowerName.includes('xray') || lowerName.includes('mri') || lowerName.includes('scan') || lowerName.includes('ultrasound')) reportType = 'Diagnostic Imaging';
    else if (lowerName.includes('prescription')) reportType = 'Prescription';
...
export const downloadReportFile = async (req: any, res: Response) => {
  try {
    const report = await Report.findOne({ _id: req.params.id, userId: req.user._id });
    if (!report) return res.status(404).json(formatResponse(false, null, 'Report not found'));
