...
export const getAnalytics = async (req: any, res: Response) => {
  try {
    ...
    assessments.forEach(a => {
      ...
      if (score >= 80) lowRiskCount++;
      else if (score >= 60) moderateRiskCount++;
      else highRiskCount++;
    });
    ...
    assessments.forEach(a => {
      if (a.lifestyle?.smoking === 'yes') factorCounts['Smoking / Tobacco']++;
      if (a.lifestyle?.alcohol === 'frequently') factorCounts['Frequent Alcohol']++;
      if (a.lifestyle?.sleepHours && a.lifestyle.sleepHours < 6) factorCounts['Short Sleep (<6 hrs)']++;
      if (a.lifestyle?.stressLevel && a.lifestyle.stressLevel >= 7) factorCounts['High Stress (>=7/10)']++;
      if (a.lifestyle?.dailyWaterIntake && a.lifestyle.dailyWaterIntake < 2) factorCounts['Low Hydration (<2L)']++;
      if (a.familyHistory?.hypertension) factorCounts['Hypertension Family Risk']++;
      if (a.familyHistory?.diabetes) factorCounts['Diabetes Family Risk']++;
    });
    ...
    const reportCategoriesMap: { [key: string]: number } = {};
    reports.forEach(r => {
...
export const exportAdminMetricsCSV = async (req: any, res: Response) => {
  try {
