...
export interface DynamicScoreResult {
  ...
  scoreChange?: number; // delta (+4, -2, etc.)
...
export const computeDynamicHealthScore = async (userId: string | mongoose.Types.ObjectId): Promise<DynamicScoreResult> => {
  ...
  else if (avgSleep > 9 && avgSleep <= 10) sleepScore = 85;
  else if (avgSleep >= 5 && avgSleep < 6) sleepScore = 60;
  else sleepScore = 40;
  ...
  if (hydrationRatio >= 0.95) hydrationScore = 95;
  else if (hydrationRatio >= 0.8) hydrationScore = 85;
  else if (hydrationRatio >= 0.6) hydrationScore = 70;
  else if (hydrationRatio >= 0.4) hydrationScore = 50;
  else hydrationScore = 30;

  // --- ACTIVITY SCORE (0-100) ---
  if (avgSteps >= 10000) activityScore = 95;
  else if (avgSteps >= 8000) activityScore = 85;
  else if (avgSteps >= 6000) activityScore = 75;
  else if (avgSteps >= 4000) activityScore = 60;
  else activityScore = 45;
  ...
  if (trackingLogs.length > 0) {
    ...
    if (latestAssessment.nutrition?.fastFoodFrequency === 'daily') nutPoints -= 15;
    else if (latestAssessment.nutrition?.fastFoodFrequency === '2-3 times/week') nutPoints -= 8;
    if (latestAssessment.nutrition?.sugarIntake === 'high') nutPoints -= 10;
  }
  nutritionScore = Math.max(20, Math.min(100, nutPoints));

  // --- LIFESTYLE & STRESS SCORE (0-100) ---
  let lifePoints = 90;
  if (avgStress >= 8) lifePoints -= 25;
  else if (avgStress >= 6) lifePoints -= 15;
  else if (avgStress >= 4) lifePoints -= 5;

  if (latestAssessment?.lifestyle?.smoking === 'yes') lifePoints -= 25;
  if (latestAssessment?.lifestyle?.alcohol === 'frequently') lifePoints -= 15;
  else if (latestAssessment?.lifestyle?.alcohol === 'occasionally') lifePoints -= 5;

  if (bmi > 0) {
    if (bmi >= 30 || bmi < 18.5) lifePoints -= 15;
    else if (bmi >= 25) lifePoints -= 8;
  }
  ...
  if (sleepScore >= 85) reasons.push('healthy restorative sleep');
  else if (sleepScore < 65) reasons.push('insufficient sleep');

  if (hydrationScore >= 85) reasons.push('consistent hydration');
  else if (hydrationScore < 65) reasons.push('low water intake');

  if (activityScore >= 85) reasons.push('strong physical activity');
  else if (activityScore < 60) reasons.push('sedentary movement');

  if (completedGoals > 0) reasons.push('completed health goals');
