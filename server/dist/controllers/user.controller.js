...
export const exportUserDataJSON = async (req, res) => {
    try {
        ...
        const [user, assessments, dailyTracking, reports, goals, preventiveEvents, familyMembers, achievements, chatMessages] = await Promise.all([
...
export const exportUserDataCSV = async (req, res) => {
    try {
        const userId = req.user._id;
...
export const purgeSelectiveData = async (req, res) => {
    try {
        const userId = req.user._id;
