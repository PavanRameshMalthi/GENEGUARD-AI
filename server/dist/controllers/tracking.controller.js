...
export const upsertDailyTracking = async (req, res) => {
    try {
        const userId = req.user._id;
        const { date = new Date().toISOString().split('T')[0], hydration, sleep, physicalActivity, nutrition, wellness } = req.body;
        ...
        if (hydrationPayload.waterGoal && hydrationPayload.waterConsumed !== undefined) {
            hydrationPayload.remainingWater = Math.max(0, Number((hydrationPayload.waterGoal - hydrationPayload.waterConsumed).toFixed(1)));
        }
        const tracking = await DailyHealthTracking.findOneAndUpdate({ userId, date }, {
            $set: {
...
export const getTrackingById = async (req, res) => {
    try {
        ...
        if (!tracking)
            return res.status(404).json(formatResponse(false, null, 'Tracking record not found'));
