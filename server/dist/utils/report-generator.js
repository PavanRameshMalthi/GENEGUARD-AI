...
<body>
  ...
  <div class="section">
    <h2>🥗 Diet Plan</h2>
    ${aiAnalysis.dietPlan.breakfast?.length ? `<h3>Breakfast</h3><ul>${aiAnalysis.dietPlan.breakfast.map((b) => `<li>${b}</li>`).join('')}</ul>` : ''}
    ${aiAnalysis.dietPlan.lunch?.length ? `<h3>Lunch</h3><ul>${aiAnalysis.dietPlan.lunch.map((l) => `<li>${l}</li>`).join('')}</ul>` : ''}
    ${aiAnalysis.dietPlan.dinner?.length ? `<h3>Dinner</h3><ul>${aiAnalysis.dietPlan.dinner.map((d) => `<li>${d}</li>`).join('')}</ul>` : ''}
    ${aiAnalysis.dietPlan.snacks?.length ? `<h3>Snacks</h3><ul>${aiAnalysis.dietPlan.snacks.map((s) => `<li>${s}</li>`).join('')}</ul>` : ''}
    ${aiAnalysis.dietPlan.avoidFoods?.length ? `<h3>Foods to Avoid</h3><ul>${aiAnalysis.dietPlan.avoidFoods.map((f) => `<li>${f}</li>`).join('')}</ul>` : ''}
    ${aiAnalysis.dietPlan.healthyFoods?.length ? `<h3>Healthy Foods</h3><ul>${aiAnalysis.dietPlan.healthyFoods.map((f) => `<li>${f}</li>`).join('')}</ul>` : ''}
  </div>` : ''}
  ...
  <div class="section">
    ...
    ${['beginner', 'intermediate', 'advanced'].map(level => {
      ...
      <table class="exercise-table">
        ...
        <tbody>${exercises.map((e) => `<tr><td>${e.name}</td><td>${e.duration}</td><td>${e.caloriesBurned}</td><td>${e.frequency}</td></tr>`).join('')}</tbody>
      </table>`;
    }).join('')}
  </div>` : ''}
  ...
  <div class="section">
    ...
    ${aiAnalysis.sleepAnalysis.tips?.length ? `<h3>Sleep Tips</h3><ul>${aiAnalysis.sleepAnalysis.tips.map((t) => `<li>${t}</li>`).join('')}</ul>` : ''}
  </div>` : ''}
  ...
  <div class="section">
    ...
    ${aiAnalysis.hydrationAnalysis.tips?.length ? `<h3>Tips</h3><ul>${aiAnalysis.hydrationAnalysis.tips.map((t) => `<li>${t}</li>`).join('')}</ul>` : ''}
  </div>` : ''}

  ${aiAnalysis?.lifestyleImprovements?.length ? `
  <div class="section">
    <h2>🌟 Lifestyle Improvements</h2>
    <ul>${aiAnalysis.lifestyleImprovements.map((l) => `<li>${l}</li>`).join('')}</ul>
  </div>` : ''}

  ${aiAnalysis?.weeklyGoals?.length ? `
  <div class="section">
    <h2>🎯 Weekly Goals</h2>
    <ul>${aiAnalysis.weeklyGoals.map((g) => `<li>${g}</li>`).join('')}</ul>
  </div>` : ''}
